...
bool Element::IsLabelOnScreen(std::wstring label)
{
	...
	for each(auto n in allElements)
		delete n;

	return false;
}

bool Element::IsSubstringLabelOnScreen(std::wstring label)
{
	auto allElements = GetScreenElements();

	for each (ElementBase* eb in allElements)
	{
		if (eb->GetLabel().find(label) != string::npos)
		{
			return true;
		}
	}
...
wchar_t* Element::GetLabel(UINT64 ElementAddress)
{
	...
	if (labelCharCount > 0 && labelCharCount < 60)
	{
		if (NextPtr)
		{
			UINT64 PtrLabel = NextPtr + Offsets::Element::Label_2;
			
			if (DereferenceSafe<UINT64>(PtrLabel) != NULL)
			{
				__try
				{
...
void Element::PrintHoveredElements()
{
	...
	while (true)
	{
		UINT64 StatePtr = DereferenceSafe<UINT64>(poeBase + g_ProtectionServer->Offsets[1]);
...
std::list<ElementBase*> Element::GetScreenElements() //working: gets all elements on screen (ex Label found: Dannig, Warrior Skald, Label found: Tane Octavius, Label found: Niko, Master of the Depths)
{
	...
	auto StatePtr = DereferenceSafe<UINT64>(poeBase + g_ProtectionServer->Offsets[1]);
	...
	if (StatePtr != NULL)
	{
		...
		if (UIRoot != NULL)
		{
			...
			if (RootElement != NULL) //todo: clean up the process of getting child elements 
			{
